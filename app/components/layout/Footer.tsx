...
export default function Footer() {
  ...
  return (
    <footer className={`relative overflow-hidden transition-colors duration-300 ${isDark ? "always-dark bg-navy text-cream" : "bg-warm text-navy"}`}>
      {/* Decorative top border */}
      <div className="h-px w-full bg-gradient-to-r from-transparent via-accent/40 to-transparent" />

      {/* Ambient glow */}
      <div className={`pointer-events-none absolute bottom-0 left-1/2 -translate-x-1/2 w-[600px] h-40 blur-3xl rounded-full ${isDark ? "bg-accent/5" : "bg-accent/8"}`} />
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 pt-10 md:pt-14 pb-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-10">
          ...
          <div className="text-center md:text-left">
            <span className={`font-playfair text-xl font-bold tracking-wide ${isDark ? "text-cream" : "text-navy"}`}>
              Valeria Gongora Torres
            </span>
            <p className={`text-xs mt-1 max-w-[220px] leading-relaxed ${isDark ? "text-cream/72" : "text-navy/70"}`}>
              {t(tr.desc)}
            </p>
          </div>
          ...
          <nav className="flex flex-wrap justify-center gap-x-5 gap-y-2.5">
            {[
              ...
              <a
                key={item.href}
                href={item.href}
                className={`text-xs uppercase tracking-widest transition-colors duration-200 ${isDark ? "text-cream/78 hover:text-cream" : "text-navy/75 hover:text-accent"}`}
              >
                {t(item.label)}
              </a>
            ))}
          ...
          <div className="flex gap-3">
            <a href={GITHUB} target="_blank" rel="noopener noreferrer"
               className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 ${
                 isDark
                   ? "border border-cream/15 text-cream/78 hover:text-cream hover:border-accent/50 hover:bg-accent/10"
                   : "border border-navy/12 text-navy/75 hover:text-accent hover:border-accent/50 hover:bg-accent/10"
               }`}>
        ...
        {/* Divider */}
        <div className={`h-px bg-gradient-to-r from-transparent to-transparent mb-6 ${isDark ? "via-cream/10" : "via-navy/10"}`} />
